...
func TestNewEventLocation(t *testing.T) {
	...
	var tests = []struct {
		...
		{entities.EventLocation{PostalCode: "00000-000"}, nil},
		{entities.EventLocation{PostalCode: "00000-000"}, nil},
	...
	for _, test := range tests {
		_, actual := entities.NewEventLocation(
			test.param.Street, test.param.District, test.param.State, test.param.City, test.param.PostalCode, test.param.Description, test.param.Number, test.param.Latitude, test.param.Longitude)
